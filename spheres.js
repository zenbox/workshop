...
function init() {
    ...
    // Körper im Raum
    // Eine Grundgeometrie
    radius = Math.random() * 0.02 * 6;
    const geometry = new THREE.IcosahedronGeometry(radius, 3);

    // 200 mal verwendet und positioniert
    for (let i = 0; i < 200; i++) {
        const object = new THREE.Mesh(
            geometry,
            new THREE.MeshLambertMaterial({ color: Math.random() * 0x220000 })
        );
        ...
        let base = 25;
        object.userData.velocity.x = base + Math.random() * 0.01 - 0.005;
